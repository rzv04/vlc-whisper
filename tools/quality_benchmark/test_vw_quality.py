...
class QualityHelpersTest(unittest.TestCase):
    ...
    def test_load_manifest_rejects_conflicting_reference_aliases(self):
        with tempfile.TemporaryDirectory() as tmp:
            manifest_file = Path(tmp) / "manifest.json"
            manifest_file.write_text(
                json.dumps(
                        ...
                        "samples": [
                            {
                                ...
                                "reference_text": "validated transcript",
                                "reference": "different transcript",
                                "duration_seconds": 1.0,
                            }
                        ],
                    }
            ...
            with self.assertRaisesRegex(ValueError, "conflicting 'reference_text' and 'reference'"):
                load_manifest(manifest_file)
