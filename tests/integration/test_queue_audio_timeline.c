...
int main(void) {
  ...
  vw_test_check_true("audio buffer creation", audio_buffer != NULL);
  if (!audio_buffer) return 1;

  vw_test_check_true("initial contiguous audio append",
  ...
  vw_test_check_true("worker queue creation", queue != NULL);
  if (!queue) {
  ...
  vw_test_check_true("payload B allocation", payload_b != NULL);
  vw_test_check_true("payload C allocation", payload_c != NULL);
  vw_test_check_true("payload D allocation", payload_d != NULL);
  vw_test_check_true("payload E allocation", payload_e != NULL);
  ...
  vw_test_check_true("queue B", vw_worker_queue_push(queue, VW_MSG_AUDIO_PCM, payload_b, len_b));
  vw_test_check_true("queue C", vw_worker_queue_push(queue, VW_MSG_AUDIO_PCM, payload_c, len_c));
  vw_test_check_true("queue D", vw_worker_queue_push(queue, VW_MSG_AUDIO_PCM, payload_d, len_d));
  vw_test_check_true("queue E with eviction", vw_worker_queue_push(queue, VW_MSG_AUDIO_PCM, payload_e, len_e));
  vw_test_check_true("queue records exactly one dropped frame",
                     vw_worker_queue_get_dropped_audio_us(queue) == (uint64_t)frame_duration_us);

  vw_worker_frame_t frame = {0};
  vw_test_check_true("pop first surviving frame", vw_worker_queue_pop(queue, &frame));
  vw_test_check_true("first surviving frame is audio", frame.type == VW_MSG_AUDIO_PCM);

  vw_msg_audio_t decoded = {0};
  vw_test_check_true("decode first surviving audio frame",
                     vw_protocol_decode_payload(VW_MSG_AUDIO_PCM, frame.payload, frame.payload_len, &decoded));
  vw_test_check_true("oldest queued frame was evicted", decoded.start_pts_us == 1200000);
  vw_test_check_true("decoded frame size is whole S16LE samples", decoded.pcm_bytes % sizeof(int16_t) == 0);

  size_t before_count = vw_audio_buffer_get_count(audio_buffer);
  float before_sample = 0.0f;
  int64_t before_pts = -1;
  vw_test_check_true("read pre-discontinuity buffer anchor",
  ...
  vw_test_check_true("allocate decoded PCM copy", decoded_pcm != NULL);
  ...
  if (!accepted) {
    ...
    int64_t after_pts = -1;
    vw_test_check_true("rejected discontinuity leaves sample count unchanged",
                       vw_audio_buffer_get_count(audio_buffer) == before_count);
    vw_test_check_true("rejected discontinuity leaves anchor readable",
                       vw_audio_buffer_get_samples(audio_buffer, &after_sample, 1, &after_pts) == 1);
    vw_test_check_true("rejected discontinuity leaves anchor unchanged", after_pts == before_pts);
  } else {
    float after_sample = 0.0f;
    int64_t after_pts = -1;
    size_t after_count = vw_audio_buffer_get_count(audio_buffer);
    vw_test_check_true("accepted discontinuity leaves readable buffered audio",
                       vw_audio_buffer_get_samples(audio_buffer, &after_sample, 1, &after_pts) == 1);
    ...
    vw_test_check_true("queue eviction must not collapse missing media time",
  ...
  return vw_test_finish("test_queue_audio_timeline");
}
