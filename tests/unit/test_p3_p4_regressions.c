...
static void test_protocol_codec_null_pointer_safety(void) {
  // VW-015 / VW-083: inner pointer NULL checks when length > 0
  ...
  vw_test_check_false("vw-015: encode rejects NULL client_version with positive length", ok);
  ...
  vw_test_check_true("vw-083: encode accepts NULL client_version with zero length", ok);
  ...
  vw_test_check_false("vw-015: encode rejects NULL worker_version with positive length", ok);

  hello_ack.worker_version = NULL;
  hello_ack.worker_version_length = 0;
  ok = vw_protocol_encode_payload(VW_MSG_HELLO_ACK, &hello_ack, buf, sizeof(buf), &encoded_len);
  vw_test_check_true("vw-083: encode accepts NULL worker_version with zero length", ok);
  ...
  vw_test_check_false("vw-015: encode rejects NULL pcm_data with positive length", ok);

  pcm.pcm_bytes = 0;
  pcm.pcm_data = NULL;
  ok = vw_protocol_encode_payload(VW_MSG_AUDIO_PCM, &pcm, buf, sizeof(buf), &encoded_len);
  vw_test_check_true("vw-083: encode accepts NULL pcm_data with zero length", ok);
}

static void test_protocol_codec_trailing_bytes(void) {
  // VW-055: decode must reject payloads with unconsumed trailing bytes
  ...
  vw_test_check_false("vw-055: decode rejects unconsumed trailing bytes", ok);
}

static void test_protocol_validate_header_bounds(void) {
  // VW-056: header->type bounds validation
  ...
  vw_test_check_true("vw-056: valid header type HELLO accepted", vw_protocol_validate_header(&hdr));

  hdr.type = VW_MSG_TRANSLATE_CTRL;
  vw_test_check_true("vw-056: valid header type TRANSLATE_CTRL accepted", vw_protocol_validate_header(&hdr));

  hdr.type = 0;
  vw_test_check_false("vw-056: invalid header type 0 rejected", vw_protocol_validate_header(&hdr));

  hdr.type = VW_MSG_TRANSLATE_CTRL + 1;
  vw_test_check_false("vw-056: invalid header type beyond upper bound rejected", vw_protocol_validate_header(&hdr));
}

static void test_protocol_validate_control_reasons(void) {
  // VW-054 & VW-101: distinct reason constants & semantic validation
  vw_msg_pause_t pause_msg = {.reason = VW_CTRL_REASON_USER_PAUSE};
  vw_test_check_true("vw-054: valid pause reason accepted", vw_protocol_validate_payload(VW_MSG_PAUSE, &pause_msg));

  pause_msg.reason = VW_CTRL_REASON_USER_RESUME;
  vw_test_check_false("vw-054: invalid pause reason (USER_RESUME) rejected",
                      vw_protocol_validate_payload(VW_MSG_PAUSE, &pause_msg));

  vw_msg_resume_t resume_msg = {.reason = VW_CTRL_REASON_USER_RESUME};
  vw_test_check_true("vw-054: valid resume reason accepted", vw_protocol_validate_payload(VW_MSG_RESUME, &resume_msg));

  resume_msg.reason = VW_CTRL_REASON_USER_PAUSE;
  vw_test_check_false("vw-054: invalid resume reason (USER_PAUSE) rejected",
                      vw_protocol_validate_payload(VW_MSG_RESUME, &resume_msg));

  vw_msg_stop_t stop_msg = {.reason = VW_CTRL_REASON_USER_STOP};
  vw_test_check_true("vw-054: valid stop reason USER_STOP accepted",
                     vw_protocol_validate_payload(VW_MSG_STOP_SESSION, &stop_msg));

  stop_msg.reason = VW_CTRL_REASON_SEEK_DISCONTINUITY;
  vw_test_check_true("vw-054: valid stop reason SEEK_DISCONTINUITY accepted",
                     vw_protocol_validate_payload(VW_MSG_STOP_SESSION, &stop_msg));

  stop_msg.reason = VW_CTRL_REASON_MEDIA_END;
  vw_test_check_true("vw-054: valid stop reason MEDIA_END accepted",
                     vw_protocol_validate_payload(VW_MSG_STOP_SESSION, &stop_msg));

  stop_msg.reason = 999;
  vw_test_check_false("vw-054: arbitrary stop reason rejected",
                      vw_protocol_validate_payload(VW_MSG_STOP_SESSION, &stop_msg));

  vw_msg_error_t err_msg = {.error_code = VW_ERROR_NONE};
  vw_test_check_true("vw-054: valid error code accepted", vw_protocol_validate_payload(VW_MSG_ERROR, &err_msg));

  err_msg.error_code = VW_ERROR_INTERNAL;
  vw_test_check_true("vw-054: valid internal error code accepted",
                     vw_protocol_validate_payload(VW_MSG_ERROR, &err_msg));

  err_msg.error_code = VW_ERROR_INTERNAL + 1;
  vw_test_check_false("vw-054: out-of-range error code rejected", vw_protocol_validate_payload(VW_MSG_ERROR, &err_msg));
}

static void test_protocol_validate_pcm_alignment(void) {
  // VW-103: reject odd pcm_bytes
  int16_t samples[4] = {0};
  vw_msg_audio_t pcm = {
      .start_pts_us = 0,
      .duration_us = 250,
  ...
  vw_test_check_true("vw-103: even pcm_bytes accepted", vw_protocol_validate_payload(VW_MSG_AUDIO_PCM, &pcm));

  pcm.pcm_bytes = 4 * sizeof(int16_t) + 1;
  vw_test_check_false("vw-103: odd pcm_bytes rejected", vw_protocol_validate_payload(VW_MSG_AUDIO_PCM, &pcm));
}

static void test_protocol_util_utf8_overlong_and_surrogates(void) {
  // VW-057: reject overlong and surrogate codepoints
  const char overlong_c0[] = "\xC0\xAF";
  size_t len = vw_utf8_safe_len(overlong_c0, 2);
  vw_test_check_true("vw-057: overlong 0xC0 rejected", len == 0);

  const char overlong_c1[] = "\xC1\x81";
  len = vw_utf8_safe_len(overlong_c1, 2);
  vw_test_check_true("vw-057: overlong 0xC1 rejected", len == 0);

  // High surrogate 0xD800 -> \xED\xA0\x80
  const char surrogate[] = "\xED\xA0\x80";
  len = vw_utf8_safe_len(surrogate, 3);
  vw_test_check_true("vw-057: surrogate sequence rejected", len == 0);
...
static void test_worker_config_default_dir(void) {
  // VW-048: fallback default model directory probe
  char dir[VW_PATH_MAX_BYTES];
  bool ok = vw_worker_config_default_model_dir(dir, sizeof(dir));
  vw_test_check_true("vw-048: default model directory probe succeeds", ok);
  vw_test_check_true("vw-048: default model directory is non-empty", dir[0] != '\0');
  vw_test_check_true("vw-048: default model directory ends with models", strstr(dir, "models") != NULL);
}
...
int main(void) {
  ...
  return vw_test_finish("test_p3_p4_regressions");
}
