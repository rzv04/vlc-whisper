...
The bridge waits only for the short bootstrap result; it never monitors the detached settings process. There is no one-second sleep, polling loop, window-ready probe, or child-lifetime monitor. If the detached settings process crashes later, that is outside the extension's responsibility. The real settings process may briefly wait on its own local single-instance socket to focus an existing window; that occurs outside VLC's realtime audio path.
...
5. **Settings Process Isolation (ADR-025):** The Qt GUI owns durable user settings and UI only. Lua uses the shell-free native bridge for bounded launch acknowledgement and never monitors the detached process; the worker retains all model-download network/verification ownership.
