...
- [x] 17c. Ahead-of-Time Source File Decoding (Look-Ahead Phase 2): Extend IPC protocol to v1.1 (`VW_CAPABILITY_SOURCE_MODE`, `source_url`, `POSITION` lead pacing), implement FFmpeg (Linux) / Media Foundation (Windows) worker demuxer, Model-Once process lifetime (`ADR-015`), process-wide `MFStartup`/`MFShutdown`, and feed pre-buffered future caption segments directly into the Step 17b SPU subpicture pipeline to achieve zero perceived transcription latency. Shipped: Protocol v1.1 (`VW_CAPABILITY_SOURCE_MODE = 1U << 3`, `source_url` in `START`, `VW_MSG_POSITION` with `current_pts_us`, `input_time_us`, `playback_rate`, `flags`), native source demuxers (`vw_source_decoder_mf.c` with Media Foundation `IMFSourceReader` on Windows, `vw_source_decoder_ffmpeg.c` with `libavformat`/`libavcodec`/`libswresample` on Linux), 30s look-ahead pacing loop in worker, plugin MRL extraction (`input_item_GetURI`) & periodic `POSITION` updates, and presenter future SPU scheduling (`start_tick = mdate() + (pts - input_time)`).
...
- [ ] 17d.1. Phrase-by-Phrase Subtitle Timing & Segmentation: Extract fine-grained internal Whisper segment offsets ($t_0$/$t_1$ centiseconds via `whisper_full_get_segment_t0/t1`) instead of concatenating the entire 8s acoustic window into a single coarse block. Emit individual timed phrases with discrete `start_pts_us`/`end_pts_us` to eliminate premature dialogue spoilers (`ADR-017`, [`docs/plans/phrase_timing_segmentation_plan.md`](file:///home/razvan/vlc-whisper/.worktrees/gemini/docs/plans/phrase_timing_segmentation_plan.md)) and achieve Daum PotPlayer / Netflix-style cadence.
