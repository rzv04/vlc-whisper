...
- [x] 17b. Native SPU Subpicture Subsystem (Look-Ahead Phase 1): Integrate `vout_RegisterSubpictureChannel` and `vout_PutSubpicture` with proper `subpicture_region_New(VLC_CODEC_TEXT)` text regions, `text_segment_New()`, `VW_WEAK` MinGW symbol linkage, and system-to-media date domain conversion (`mdate() - input_time`). Shipped: private SPU subpicture channel registration with OSD fallback on failure; structured `subpicture_t` carrying `video_format_Init(&fmt, VLC_CODEC_TEXT)` and `text_segment_New` text; bottom-center alignment; media PTS-to-system tick conversion (`mdate() + (pts_us - input_time_us)`); seek/blank channel flushing via `vout_FlushSubpictureChannel(vout, channel_id)`; shared `VW_WEAK` macro in `vw_platform.h`; extended `libvlccore.def` exports for SPU and `mdate`.
