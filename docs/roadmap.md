...
- [ ] 17e.1. Third-Party VAD Integration & Silence-Aware Gating: Re-implement speech activity detection by integrating a dedicated third-party voice detection model (e.g. Silero VAD / WebRTC VAD) to replace raw RMS energy thresholding (`vw_vad_detect_speech_energy`). Accurately classify human vocal formants vs background music, ambient noise, and sound effects; suppress hallucinated phantom cues on silence/music (`[MUSIC]`, `[APPLAUSE]`); trigger Whisper inference only upon verified speech presence and gate segments using `no_speech_prob` to eliminate spurious caption bursts and unnecessary inference load.
