...
- [ ] 21a. **Research & feasibility spike — Settings GUI process architecture (ADR-022)**: determine whether the settings/control GUI ships as (a) the standalone `vlc-whisper-settings.exe` per ADR-011, (b) a VLC interface/extension plugin inside the ensemble, or (c) an in-DLL dialog; and define how it connects to the *running* plugin DLL — candidate control channels: new control messages on the existing authenticated worker-style IPC pipe, a separate local socket owned by the plugin, or config-file + session-restart handshake. Deliverables: ADR-022 decision, `docs/plans/gui_translation_research_plan.md` Phase 1 findings, updated source-layout ownership rows. ([plan](file:///home/razvan/vlc-whisper/.worktrees/gemini/docs/plans/gui_translation_research_plan.md))
- [ ] 21b. **Research spike — real-time subtitle translation service (Daum PotPlayer parity) (ADR-023)**: study how PotPlayer achieves real-time Google translation of transcripts (per-segment web/API requests emitted as Whisper finalizes each cue); evaluate per-user options that avoid shipping a shared key: Google Cloud Translation API free tier (~500k chars/month per billing account), DeepL API Free tier, or user-supplied BYO key stored locally. Hard constraint: product privacy boundary (`product.md`) forbids cloud calls by default — translation must be strictly user-opt-in with a required product-boundary amendment if shipped. Deliverables: ADR-023 decision (provider, key custody, request shape, latency budget ≤1s per final cue), plan Phase 2 findings. ([plan](file:///home/razvan/vlc-whisper/.worktrees/gemini/docs/plans/gui_translation_research_plan.md))
- [ ] 21c. Settings GUI implementation per ADR-022 outcome with PotPlayer-parity minimum feature set: inference engine selector (`auto` / Vulkan GPU / CPU with detected-backend status), model picker (`tiny`, `base`, `large` + language variants per manifest), caption language selector (auto-detect, `en`, `ro`, `tr`, full whisper language list), CPU thread-count spinner with documented default (= hardware concurrency capped by whisper guidance). Settings persist and reach the running worker via the ADR-022 control channel (live-apply where safe, worker restart where required).
- [ ] 21d. Model management implementation: download/provision UI for selected models (reusing the sha256-pinned provisioning flow from `cmake/vw_provision_model.cmake`), model path discovery, documented RAM/CPU tradeoffs, tiny fallback.
- [ ] 22. Multilingual models (`small`, `medium`, `large`) and automatic language detection.
- [ ] 23. Subtitle translation implementation per ADR-023 outcome: opt-in UI toggle, per-final-cue translation request, display strategy (translated line appended below original), strict failure degradation to untranslated captions, zero network when disabled.
- [ ] 24. Release documentation: troubleshooting, privacy statement (including the opt-in translation data-flow disclosure), uninstall guide, and bug report templates.
- [ ] 25. Benchmark suite and performance output metrics: measure Whisper inference latency across backends (CPU vs Vulkan GPU), measure Silero GGML VAD evaluation overhead and real-time factor across 6s–24s windows, track audio queue high-water marks, and calculate processing speed ratios.
