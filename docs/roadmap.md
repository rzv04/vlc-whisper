...
- [ ] 14b. Plugin real-time streaming + worker thread split: extend `vw_worker_client` with a send API (`START`/`AUDIO`/`STOP`, session and sequence tracking), drain the SPSC queue on a background sender thread to feed `AUDIO` frames across IPC in real time, drain worker frames, and degrade to passthrough gracefully on pipe death; split `vw_worker.c` into a decoupled IPC reader thread + inference worker thread (ADR-013) so inference never blocks the read loop.
