...
- [x] 17e.1. Third-Party VAD Integration & Silence-Aware Gating: Re-implement speech activity detection by integrating a dedicated third-party voice detection model (e.g. Silero VAD / WebRTC VAD) to replace raw RMS energy thresholding (`vw_vad_detect_speech_energy`). Accurately classify human vocal formants vs background music, ambient noise, and sound effects; suppress hallucinated phantom cues on silence/music (`[MUSIC]`, `[APPLAUSE]`); trigger Whisper inference only upon verified speech presence and gate segments using `no_speech_prob` to eliminate spurious caption bursts and unnecessary inference load. Shipped: vendored Silero GGML VAD integration (`whisper_vad_*` / `vw_vad`) with RMS Energy fallback across all 3 worker call sites; `--vad-model` CLI parameter and auto-discovery (`ggml-silero-vad.bin`); `wparams.no_speech_thold = 0.60f`, `suppress_nst = true`, and segment `no_speech_prob` gating; modular `vw_hallucination_filter` for non-speech sound tags and isolated punctuation stripping; LSTM state resets on discontinuity events; Strategy C VAD-guided non-overlapping audio chunking (`vw_vad_find_chunk_boundary`) for lookahead mode, eliminating duplicate/stuttering subtitles and saving 75% inference compute (`ADR-020`); POSIX and Windows model download scripts; comprehensive unit tests and documentation (`ADR-019`, `ADR-020`).
- [ ] 17e.2. Subtitle Pacing, Minimum Reading Floor & Decoding Optimization: Enforce a minimum display duration floor ($\ge 1.0\,\text{s}$) in SPU caption presentation to eliminate unreadable sub-second flash cues; configure Whisper decoding parameters (temperature fallback, repetition penalty, and entropy gating) for stable, deterministic acoustic output across shifting window hops.
