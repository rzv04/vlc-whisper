...
- [ ] 14a. Worker audio pipeline: implement real `vw_whisper_engine` (model load, warmup, transcription) and `vw_audio_buffer` (S16LE→float32 append, bounded drop-oldest), handle `START`/`AUDIO`/`STOP` in `vw_worker.c` with ADR-013 reader/inference threads, 8 s window / 2 s hop windowing and energy-VAD gating, and emit `CAPTION_SEGMENT` frames.
- [ ] 14b. Plugin real-time streaming: extend `vw_worker_client` with a send API (`START`/`AUDIO`/`STOP`, session and sequence tracking), drain the SPSC queue on a background sender thread to feed `AUDIO` frames across IPC in real time, drain worker frames, and degrade to passthrough gracefully on pipe death.
