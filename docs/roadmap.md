...
- [ ] 17e. Transcription Quality & Stability Pass (ordered): raise caption quality and streaming stability, keeping the 8 s window / 2 s hop geometry (latency bounded by 17c/17d).
  1. **VAD + silence-aware windowing** (highest impact): transcribe only when speech is detected (replace/augment the current energy threshold in `vw_vad_detect_speech_energy` — evaluate WebRTC VAD vs adaptive-energy vs whisper voice detection); suppress hallucinated cues on silence/music (`[MUSIC]`); trigger inference on speech onset rather than a fixed 8 s cadence; fewer spurious cues -> less overlap re-transcription pressure on the time-coverage dedup.
  2. **Model selection**: user-selectable `base.en`/`small.en` (larger models recognize more stably -> fewer prefix/suffix re-recognition variants -> less instability at the source); `--model`/`model-path` already plumbed; add discovery + documented RAM/CPU/inference-time tradeoff; keep tiny.en as fallback.
  3. **Beam-search decoding** (`WHISPER_SAMPLING_BEAM_SEARCH`, `beam_size`, temperature fallback): steadier hypotheses across overlapping windows; keep greedy decode as fallback.
  4. **Word-level timestamps** (revisit `whisper_full_get_token_data` with `token_timestamps`): exact word boundaries for tighter phrase segmentation and dedup trims; re-opens the ADR-018 edge-case surface — gate behind the coverage model.
