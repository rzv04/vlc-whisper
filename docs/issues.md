...
- **Root cause**: The Linux ancestor walk in `vw_plugin_resolve_worker_path()` (`plugin/src/vw_whisper_module.c`) starts at `up = 1`, so the plugin's own directory is never probed. The Windows branch starts at `up = 0` and finds a co-located worker; Linux does not.
