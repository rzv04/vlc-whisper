...
- `tests/unit/test_vad.c` (17e.1): pure silence, low-level ambient static (<0.005 RMS), speech tone/wave RMS energy detection, NULL context fallback, partial window sample counts, NULL safety for reset/free, and model-gated Silero VAD GGML inference on real 16kHz speech fixtures (`jfk.wav`).
