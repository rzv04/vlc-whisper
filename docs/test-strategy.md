...
- `cmake/Packaging.cmake` & `cmake/vlc_whisper_installer.nsi.in` (18): Standalone Windows NSIS setup installer compilation (`makensis`), 64-bit VLC auto-discovery (`HKLM\Software\VideoLAN\VLC`), automated plugin deployment (`<VLC>\plugins\audio_filter\`), worker/models deployment, automatic `vlc-cache-gen.exe` cache regeneration, uninstaller clean state rollback, CPack release ZIP generation, and end-to-end acceptance testing covering local media (.mp4, .mkv, .mp3, .flac) and live network streams.
