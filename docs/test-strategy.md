...
- `cmake/vw_packaging.cmake` & `cmake/vw_installer.nsi.in`: Standalone Windows NSIS setup installer compilation (`makensis`), 64-bit VLC auto-discovery (`HKLM\Software\VideoLAN\VLC`), locale-independent VLC process detection with explicit nonzero abort on an unresolved running-process check, automated plugin deployment (`<VLC>\plugins\audio_filter\`), GPU plus isolated CPU-fallback worker staging, model deployment, ownership-filtered worker shutdown, reboot-safe locked-file replacement, project-owned notice files, uninstaller cleanup, CPack release ZIP generation, and end-to-end acceptance testing covering local media (.mp4, .mkv, .mp3, .flac) and live network streams.
...
**Current validation limits on this branch:** Native Linux configure/build, all 25 native tests, and the CPU-forced Valgrind memcheck gate are the reproducible local checks. Windows cross-build and clean Windows VLC/installer acceptance still require the Windows toolchain/VM; no Windows runtime result is claimed here.
