...
- **Windows Installer (NSIS)**: Standalone installer (`vlc-whisper-0.1.0-win64-setup.exe`, version `0.1.0`) auto-detects VLC 64-bit installation paths from `HKLM\Software\VideoLAN\VLC`, validates 64-bit `vlc.exe` via `GetBinaryTypeW`, installs the plugin DLL to `<VLC>\plugins\audio_filter\`, worker executable(s), and the pinned multilingual tiny plus Silero VAD models. The production `windows-x64-release` preset sets `VW_REQUIRE_VULKAN=ON`, so missing Vulkan SDK/`glslc` is fatal instead of silently converting the official GPU build into a CPU-only artifact. GPU packages stage `vlc-whisper-worker.exe` plus an isolated `VW_WITH_VULKAN=OFF` CPU fallback; explicit CPU-only installers remove (or schedule reboot-time removal of) an older canonical GPU worker so discovery cannot select stale code first. Before NSIS runs, `vw_check_workers.cmake` requires the expected worker set, plugin DLL, tiny model, and VAD model and verifies both model SHA-256 values. Installer provisioning also hashes already-present untracked model files before accepting them. The portable CPack archive uses an explicit model allowlist rather than `models/*.bin`, so unrelated local development models cannot leak into a release. Installation invalidates VLC's plugin cache by deleting `plugins.dat` (VLC rebuilds on next launch), registers an uninstaller, removes only the installing user's stored app-owned `%LOCALAPPDATA%\vlc-whisper\models` directory during uninstall without touching root `LICENSE`/`THIRD_PARTY_NOTICES.md`, and creates shortcuts (`vlc.exe --audio-filter=vlc_whisper`).
