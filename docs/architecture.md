...
- **Multi-Tier Voice Activity Detection & Silence Gating (Step 17e.1, `ADR-019`)**:
  - **Tier 1 (Pre-Inference VAD)**: Employs vendored Silero GGML VAD (`whisper_vad_detect_speech` / `whisper_vad_segments_from_probs`) across all three worker audio ingestion sites (live PCM stream, lookahead full window, and lookahead trailing EOF window). Auto-discovers `ggml-silero-vad.bin` in the model directory alongside `ggml-tiny.en.bin` or via CLI `--vad-model <path>`, gracefully falling back to zero-config RMS Energy VAD (`0.01f`) if absent. Silent/music windows skip Whisper inference completely, cutting idle CPU/GPU usage by up to 80%.
