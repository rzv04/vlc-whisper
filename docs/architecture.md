...
- **Validation limits**: Windows pipe random endpoint (32-byte CSPRNG hex, bounded `\\.\pipe\vlc-whisper-<64hex>`), same-user ACL + `FILE_FLAG_FIRST_PIPE_INSTANCE` first-instance protection, and trusted absolute spawn (no bare CWD fallback; auto backend retries CPU worker via absolute path) are implemented. Sequence is `uint64_t` monotonic per direction (wraparound practically impossible vs `uint32_t`). Live discontinuity creates new session epoch (`STOP`/`START`), source uses `POSITION` with `SEEK` flag and 5 s / 500 ms gating, including media-swap and rate-change re-anchoring. Language validation uses `whisper_lang_id` (rejects `auto` visibly). Linux monotonic failure returns error sentinel, never `time(NULL)`. Manual VAD download verifies manifest SHA-256 in `.part` and cleans on mismatch. Windows VM smoke and full Valgrind clean gate (requires `VW_FORCE_CPU=1` to suppress Vulkan false positives) remain pending on this commit.
