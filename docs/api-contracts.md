...
| `E_TRANSLATION_PROVIDER` | the terminal attempted fallback ended in a provider-side rejection/error; earlier fallback tiers may have failed for other causes, and optional bounded provider status may be included | recoverable: render the source cue, emit provider blame when logging is enabled, and increment the provider-failure benchmark aggregate |
| `E_TRANSLATION_TRANSPORT` | the terminal attempted fallback failed at the network/transport layer; earlier fallback tiers may have failed for other causes | recoverable: render the source cue, emit transport blame when logging is enabled, and increment the transport-failure benchmark aggregate |
| `E_TRANSLATION_PARSE` | the terminal attempted fallback returned a response that could not yield valid translated text; earlier fallback tiers may have failed for other causes | recoverable: render the source cue, emit parse blame when logging is enabled, and increment the parse-failure benchmark aggregate |
...
The required contract is reject-on-overflow for every identity argument. Identity arguments (`--pipe`, `--vad-model`, and `--log-file`) reject overlong inputs rather than truncating. `--log-file` implies diagnostics. VAD resolution prefers explicit path, then model-related/install/user candidates, then documented fallback behavior. Backend reporting uses actual inference outcome.
