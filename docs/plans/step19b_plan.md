...
- Known limitations/follow-ups: all four settings currently apply via respawn (live per-call apply for language/threads without restart is a future optimization — no behavior difference except gap); auto-detect language deliberately omitted; lazy model provisioning is 19c.
