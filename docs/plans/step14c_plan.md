...
1. **Worker frame queue (new)** — `worker/include/vw_worker_queue.h` + `worker/src/vw_worker_queue.c` (no existing equivalent: `vw_spsc_queue` in plugin/include/vw_queue.h is fixed-type for `vw_audio_chunk_t` and lives plugin-side). SPSC, C11 `stdatomic`, no locks:
   ...
   ```
   Push takes ownership of `payload` and frees it if dropped. Drop policy: on full, evict the OLDEST frame whose `type == VW_MSG_AUDIO_PCM`, decode its payload via `vw_protocol_decode_payload` to read `duration_us` (fall back to 0 on decode failure), add it to `dropped_audio_us`; control frames are NEVER dropped to make room for audio. If no AUDIO frame is evictable (all-control queue), evict only a control the incoming supersedes or the worker never needs: PAUSE/RESUME (stateless no-ops), a same-type control, — for SHUTDOWN anything — or, for a required incoming (START/STOP), the oldest non-SHUTDOWN control; a queued SHUTDOWN is never evicted by a non-SHUTDOWN incoming. If nothing is evictable, drop the incoming control (counted only if AUDIO); a required incoming is dropped only into an all-SHUTDOWN queue (worker exiting anyway). Every header function gets the required 20–30 word comment (AGENTS.md rule 11).
...
- Architectural decisions (standing questions, all verified against source):
  ...
  - **Worker thread split**: reader thread = only reader (pipe drained continuously, ADR-013); main loop = session+inference, only writer (HELLO_ACK/STARTED/ERROR/SEGMENT replies). Frames flow reader → bounded SPSC frame queue → main loop; controls never evicted for audio, overflow evicts only PAUSE/RESUME, same-type, or oldest non-SHUTDOWN for a required incoming, audio drop-oldest with `dropped_audio_us` counter.
...
7. **Worker stdout/stderr was invisible (crash diagnosis blind)** — the worker is spawned with `CREATE_NO_WINDOW` and no console, so whisper's own output, `vw_log_event` lines, and any crash text went nowhere. The Win32 spawn now redirects worker stdout+stderr to `%TEMP%\vlc-whisper-worker.log` (inheritable handle + `STARTF_USESTDHANDLES`, only for the worker binary; other spawns untouched). Plugin logs `PLUGIN_WORKER_LAUNCH` (resolved path), `PLUGIN_WORKER_CONNECT`, and `PLUGIN_WORKER_DEAD` with the failing call + chunk count. Worker logs lifecycle (`WORKER_LIFECYCLE`), engine state (`WORKER_ENGINE`), session events (`WORKER_SESSION`), inference per window (`WORKER_INFERENCE`), and reader fatal (`WORKER_READER`).
8. **Real session-killer: off-by-one in AUDIO validation (14a-era bug, exposed by 14c)** — the worker exits rc=0 mid-playback (no SHUTDOWN logged → the silent invalid-payload `break`), closing the pipe; the plugin then reports `PLUGIN_WORKER_DEAD receive_frame fatal`. Root cause: `vw_audio_capture_process_block` computed `duration_us = trunc(frames × 62.5)` while `pcm_bytes = frames × 2`; the worker's `vw_protocol_validate_payload` requires `pcm_bytes == trunc(duration_us × 32 / 1000)`. For **odd frame counts** (partial VLC blocks), `trunc(f×62.5)×32/1000 = 2f−1 ≠ 2f` — a one-byte mismatch that killed the session (~27 chunks ≈ 13.8 s in, when the first odd partial block arrived). Fixed two ways: (a) capture derives `duration_us = ceil(frames × 62.5)` so the contract holds exactly for every chunk size (verified 0 mismatches over all 1..8192); (b) worker validation now accepts ±1 byte per the api-contracts "whole-sample rounding" allowance, so a future producer rounding the other way cannot kill a session. Regression tests added for both.
