...
- **In scope**:
  ...
  5. Worker session pipeline in `worker/src/vw_worker.c`:
     ...
     - Decoupled reader thread + inference worker thread (`ADR-013`).
...
- **Ownership/Threading Model**:
  - Main IPC thread reads transport frames into internal SPSC queue (`ADR-013`).
  - Worker inference thread pops PCM chunks, appends to float32 ring buffer, executes VAD/inference, and sends output frames over IPC.
...
- [ ] `vw_worker` handles `START` -> `STARTED`, validates `session_id`, ingests `AUDIO` frames via decoupled reader thread (`ADR-013`), executes 8s windowing with 2s hop, and emits `CAPTION_SEGMENT` frames.
...
- [ ] No blocking calls on IPC transport read thread.
