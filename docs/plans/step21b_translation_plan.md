...
- **In scope**:
  ...
  3. **Timing & Realtime Fallback Handling (`worker/src/vw_worker.c` & `plugin/src/vw_caption_presenter.c`)**:
  ...
  4. **Protocol v1.5 Wire Contract (`protocol/include/vw_protocol_types.h` & `protocol/src/vw_protocol_codec.c`)**:
     - Extended `vw_caption_segment_t` or added `vw_msg_translated_segment_t` with `translated_text_utf8` and `translated_text_bytes`.
  ...
  5. **Automated Unit & Contract Tests (`tests/unit/test_translate.c`)**:
...
|  |  - Finalized Cues        |       |  - 3-Tier Keyless Fallback Engine    |  |
...
- **Worker Outputs**:
  - `VW_MSG_CAPTION_SEGMENT` containing primary text and optional translated subtitle payload.
- **Presenter Outputs**:
...
- **Plugin Sender Thread**: Reads translated segment frames from worker IPC and dispatches to caption presenter.
- **Worker Inference Thread**: Emits raw finalized Whisper segments into the worker's internal translation queue.
- **Worker Translation Thread**: Dedicated background worker thread that drains the translation queue, executes HTTPS requests with an 800ms deadline, and passes translated results to the IPC output serializer.
...
- **Compatible Minor Bump (Protocol v1.5)**:
  ...
  - Extend `vw_caption_segment_t` with `uint16_t translated_bytes` and `char translated_text[VW_MAX_CAPTION_TEXT_BYTES]`.
...
- [ ] When network is offline or translation fails, original Whisper subtitles continue rendering without delay or stutter.
...
2. **Mock Network Tests**:
   - Simulate network timeouts and verify fallback to untranslated cues within $\le 800\text{ ms}$.
...
- [ ] Translation queue is strictly bounded with timeout fallbacks.
